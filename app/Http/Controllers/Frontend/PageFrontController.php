...
class PageFrontController extends Controller
{
    public function home()
    {
        ...
        $homeSectionAfter = HomeSection::active()->where('after_product', true)->orderBy('is_sort', 'ASC')->get();
        $homeSectionBefore = HomeSection::active()->where('after_product', false)->orderBy('is_sort', 'ASC')->get();
    	...
    	return view('frontend.home', [
    		...
    		'products' => $products,
            'homeSectionAfter' => $homeSectionAfter,
            'homeSectionBefore' => $homeSectionBefore,
            'blogs' => $blogs,
            'faqs' => $faqs,
            'seo' => [
    ...
    public function product($slug)
    {
        ...
        $produkBenefits = ProdukBenefit::active()->where('produk_id', $data->id)->orderBy('no', 'ASC')->get();
        $produkPatens = ProdukPaten::active()->where('produk_id', $data->id)->orderBy('id', 'ASC')->get();
        $produkResolves = ProdukResolve::active()->where('produk_id', $data->id)->orderBy('no', 'ASC')->get();
        if(isset($data->view)){
            $view = $data->view + 1;
    	...
    	return view('frontend.product', [
            'data' => $data,
            'produkBenefits' => $produkBenefits,
            'produkPatens' => $produkPatens,
            'produkResolves' => $produkResolves,
            'seo' => [
                'title' => $data->name,
    ...
    public function news()
    {
        ...
        $datas = $data->orderBy('created_at', 'DESC')->paginate(1);
