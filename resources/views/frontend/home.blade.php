...
@section('content')
<!-- Hero Start -->
<section class="mt-70 bg-half-260 bg-primary d-table w-100" style="background: url('{{ asset('image/'.$home_slider->banner) }}') center center;">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="title-heading text-center mt-4" style="color: rgb(150, 149, 156)">
                    <h1 class="display-4 title-dark font-weight-bold mb-3">{{ $home_slider->title }}</h1>
                    <p class="para-desc mx-auto">{{ $home_slider->description }}</p>
                    <div class="mt-4 pt-2">
...
@if(count($homeSectionBefore) > 0)
    @foreach($homeSectionBefore as $homeSection)
...
@if(count($homeSectionAfter) > 0)
    @foreach($homeSectionAfter as $homeSection)
    <section class="section @if($homeSection->bg_color == 2) bg-light @endif">
        <div class="container">
            <div class="row align-items-center">
                @if($homeSection->img_loc == 1)
