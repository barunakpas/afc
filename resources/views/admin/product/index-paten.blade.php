...
@section('content')
    ...
    <div class="row">
      <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">{{ $product->name }} - Section 6 (Paten) </h4>
            <p class="card-description">
              list <code>Section 6</code> <button class="btn btn-success right" data-toggle="modal" data-target="#modalProduct"><span class="mdi mdi-plus"></span> Add New</button>
            </p>
              ...
              <tbody>
                ...
                <tr id="tr-{{$data->id}}">
                  ...
                  <td>
                    ...
                    @endif
                  <td>
                    <a onclick="edit('{{$data->id}}','{{$data->description}}','{{$data->title}}','{{$data->active}}','{{$data->image}}')"><label class="badge badge-primary"><i class="mdi mdi-pencil"></i></label></a>  
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="formProduct" method="post" action="{{ route('product_paten_create', $product->id) }}" enctype="multipart/form-data">
            @csrf
...
@section('scripts')
  <script type="text/javascript">
    $('.dropify').dropify();
    function edit(id, description, title, active, image) {
      var url = '{{ route("product_paten_update", ":id") }}';
    ...
    function messageDelete(id, name) {
      var url = '{{ route("product_paten_delete", ":id") }}';
      url = url.replace(':id', id);
      swal({
          title: 'Are you sure?',
                ...
                success: function (response)
                {
                  if(response.status == 200){
                    swal({
                      ...
                      text: 'Delete Product Paten '+name+' Success',
                      icon: 'success'
                    })
                    $("#tr-"+id).remove();
                  }else{
                    swal({
                      title: 'Delete Failed!',
                      text: 'Delete Product Paten '+name+' Failed',
                      icon: 'error'
                    })
                  }
                },
